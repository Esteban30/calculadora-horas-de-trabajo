...
<div class="container-fluid">
  <div class="container-fluid" style="width:100%;height:120%;">
    ...
    <form action="{{url('/crearclientes')}}" onsubmit='return validar()' enctype="multipart/form-data" method="post">
      {{csrf_field() }}
      <div class="row">
        <div class="col-6">
      ...
      <table class="table table-light" id="tablaClientes">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{$cli->telefono}}</td>
            <td>
            <button type="button" onclick="editar({{$cli->id}})"  class="btn btn-primary"  data-toggle="modal" data-target="#editarGrupo"><i class="fa fa-pencil" aria-hidden="true"></i></button>  
            |
              <form method="post" action="{{url('/clientes/'.$cli->id)}}">
                {{csrf_field()}}
                {{method_field('DELETE')}}
                <button type="submit" class=" btn btn-danger" onclick="return confirm('¿ Seguro que quieres borrar?');">Borrar</button>
              </form>
            </td>
          </tr>
          @endforeach
...
<div class="modal fade" id="editarGrupo" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      ...
      <div class="row">
        <div class="col-6">
          <label for="Identificacion">{{'Identificacion'}}</label>
          <input type="number" name="identificacion_upd" id="identificacion_upd" class="form-control" placeholder="Identificación">
        </div>
        <div class="col">
        <label for="Genero">{{'Genero'}}</label>
